...
/** JSON codec facade for ASG nodes. Handles encoding ASG nodes to JSON strings and decoding JSON strings back to ASG
  * nodes. The JSON format matches the AsciiDoc TCK's expected ASG JSON schema.
  *
  * Uses manual encoding/decoding because the `name` and `type` discriminator fields come from sealed abstract class
  * constructor vals, which Schema.derived doesn't handle.
  */
object AsgCodecs:
    ...
    private def encodeInline(inline: Inline): String = inline match
        ...
        case CharRef(value, location) =>
            s"""{"name":"charref","type":"string","value":${encodeJson(value)},"location":${encodeLocation(
                    location
                )}}"""
        ...
        case Span(variant, form, inlines, location) =>
            s"""{"name":"span","type":"inline","variant":${encodeJson(variant)},"form":${encodeJson(
                    form
                )},"inlines":${encodeInlines(inlines)},"location":${encodeLocation(location)}}"""
        case Ref(variant, target, inlines, location) =>
            s"""{"name":"ref","type":"inline","variant":${encodeJson(variant)},"target":${encodeJson(
                    target
                )},"inlines":${encodeInlines(inlines)},"location":${encodeLocation(location)}}"""
    ...
    private def encodeBlock(block: Block): String =
        ...
        block match
            case Section(_, _, _, _, level, blocks, location) =>
                s"""{"name":"section","type":"block",${common}"level":${encodeJson(level)},"blocks":${encodeBlocks(
                        blocks
                    )},"location":${encodeLocation(location)}}"""
            case Heading(_, _, _, _, level, location) =>
                s"""{"name":"heading","type":"block",${common}"level":${encodeJson(level)},"location":${encodeLocation(
                        location
                    )}}"""
            case p: Paragraph =>
                ...
                s"""{"name":"paragraph","type":"block",${common}${extras}"inlines":${encodeInlines(
                        p.inlines
                    )},"location":${encodeLocation(p.location)}}"""
            case l: Listing =>
                val extras = new StringBuilder
                l.form.foreach(v => extras.append(s""""form":${encodeJson(v)},"""))
                l.delimiter.foreach(v => extras.append(s""""delimiter":${encodeJson(v)},"""))
                s"""{"name":"listing","type":"block",${common}${extras}"inlines":${encodeInlines(
                        l.inlines
            ...
            case l: Literal =>
                ...
                l.form.foreach(v => extras.append(s""""form":${encodeJson(v)},"""))
                l.delimiter.foreach(v => extras.append(s""""delimiter":${encodeJson(v)},"""))
                s"""{"name":"literal","type":"block",${common}${extras}"inlines":${encodeInlines(
                        l.inlines
                    )},"location":${encodeLocation(l.location)}}"""
            case p: Pass =>
                val extras = new StringBuilder
                p.form.foreach(v => extras.append(s""""form":${encodeJson(v)},"""))
                p.delimiter.foreach(v => extras.append(s""""delimiter":${encodeJson(v)},"""))
                s"""{"name":"pass","type":"block",${common}${extras}"inlines":${encodeInlines(
                        p.inlines
                    )},"location":${encodeLocation(p.location)}}"""
            case s: Stem =>
                val extras = new StringBuilder
                ...
                s"""{"name":"stem","type":"block",${common}${extras}"inlines":${encodeInlines(
                        s.inlines
                    )},"location":${encodeLocation(s.location)}}"""
            case v: Verse =>
                val extras = new StringBuilder
                v.form.foreach(ve => extras.append(s""""form":${encodeJson(ve)},"""))
                v.delimiter.foreach(ve => extras.append(s""""delimiter":${encodeJson(ve)},"""))
                s"""{"name":"verse","type":"block",${common}${extras}"inlines":${encodeInlines(
                        v.inlines
                    )},"location":${encodeLocation(v.location)}}"""
            case s: Sidebar =>
                s"""{"name":"sidebar","type":"block",${common}"form":${encodeJson(s.form)},"delimiter":${encodeJson(
                        s.delimiter
                    )},"blocks":${encodeBlocks(s.blocks)},"location":${encodeLocation(s.location)}}"""
            case e: Example =>
                s"""{"name":"example","type":"block",${common}"form":${encodeJson(e.form)},"delimiter":${encodeJson(
                        e.delimiter
                    )},"blocks":${encodeBlocks(e.blocks)},"location":${encodeLocation(e.location)}}"""
            case a: Admonition =>
                s"""{"name":"admonition","type":"block",${common}"form":${encodeJson(a.form)},"delimiter":${encodeJson(
                        a.delimiter
                    )},"variant":${encodeJson(a.variant)},"blocks":${encodeBlocks(
                        a.blocks
                    )},"location":${encodeLocation(a.location)}}"""
            case o: Open =>
                s"""{"name":"open","type":"block",${common}"form":${encodeJson(o.form)},"delimiter":${encodeJson(
                        o.delimiter
                    )},"blocks":${encodeBlocks(o.blocks)},"location":${encodeLocation(o.location)}}"""
            case q: Quote =>
                s"""{"name":"quote","type":"block",${common}"form":${encodeJson(q.form)},"delimiter":${encodeJson(
                        q.delimiter
                    )},"blocks":${encodeBlocks(q.blocks)},"location":${encodeLocation(q.location)}}"""
            case l: List =>
                s"""{"name":"list","type":"block",${common}"variant":${encodeJson(l.variant)},"marker":${encodeJson(
                        l.marker
                    )},"items":${l.items.map(encodeBlock).mkString("[", ",", "]")},"location":${encodeLocation(
                        l.location
                    )}}"""
            case d: DList =>
                s"""{"name":"dlist","type":"block",${common}"marker":${encodeJson(d.marker)},"items":${d.items
                        .map(encodeBlock)
                        .mkString("[", ",", "]")},"location":${encodeLocation(d.location)}}"""
            case li: ListItem =>
                val extras = new StringBuilder
                if li.blocks.nonEmpty then extras.append(s""""blocks":${encodeBlocks(li.blocks)},""")
                s"""{"name":"listItem","type":"block",${common}"marker":${encodeJson(
                        li.marker
                    )},"principal":${encodeInlines(li.principal)},${extras}"location":${encodeLocation(li.location)}}"""
            case di: DListItem =>
                val extras   = new StringBuilder
                ...
                s"""{"name":"dlistItem","type":"block",${common}"marker":${encodeJson(
                        di.marker
                    )},"terms":$termsStr,${extras}"location":${encodeLocation(di.location)}}"""
            case b: Break =>
                s"""{"name":"break","type":"block",${common}"variant":${encodeJson(
                        b.variant
                    )},"location":${encodeLocation(b.location)}}"""
            case a: Audio =>
                val extras = new StringBuilder
                a.target.foreach(v => extras.append(s""""target":${encodeJson(v)},"""))
                s"""{"name":"audio","type":"block",${common}"form":${encodeJson(
                        a.form
                    )},${extras}"location":${encodeLocation(a.location)}}"""
            case v: Video =>
                val extras = new StringBuilder
                v.target.foreach(ve => extras.append(s""""target":${encodeJson(ve)},"""))
                s"""{"name":"video","type":"block",${common}"form":${encodeJson(
                        v.form
                    )},${extras}"location":${encodeLocation(v.location)}}"""
            case i: Image =>
                val extras = new StringBuilder
                i.target.foreach(v => extras.append(s""""target":${encodeJson(v)},"""))
                s"""{"name":"image","type":"block",${common}"form":${encodeJson(
                        i.form
                    )},${extras}"location":${encodeLocation(i.location)}}"""
            case t: Toc =>
                val extras = new StringBuilder
                t.target.foreach(v => extras.append(s""""target":${encodeJson(v)},"""))
                s"""{"name":"toc","type":"block",${common}"form":${encodeJson(
                        t.form
                    )},${extras}"location":${encodeLocation(t.location)}}"""

    private def encodeNode(node: Node): String = node match
        case d: Document =>
            val extras = new StringBuilder
            d.attributes.foreach { attrs =>
                val attrEntries = attrs
                    .map { (k, v) =>
            ...
            s"""{"name":"document","type":"block",${extras}"blocks":${encodeBlocks(
                    d.blocks
                )},"location":${encodeLocation(d.location)}}"""
    ...
    private def decodeBlockCommon(
        fields: Chunk[(String, Json)]
    ): Either[String, (Option[String], Option[Chunk[Inline]], Option[Chunk[Inline]], Option[BlockMetadata])] =
        for
    ...
    private def decodeAttributesOpt(
        fields: Chunk[(String, Json)]
    ): Either[String, Option[Map[String, Option[String]]]] =
        fields.collectFirst {
            case (k, v) if k == "attributes" =>
