...
trait AstVisitor[A]:
    ...
    def visitLiteralBlock(node: LiteralBlock): A   = visitBlock(node)
    def visitSidebarBlock(node: SidebarBlock): A   = visitBlock(node)
    def visitExampleBlock(node: ExampleBlock): A   = visitBlock(node)
    def visitQuoteBlock(node: QuoteBlock): A       = visitBlock(node)
    def visitOpenBlock(node: OpenBlock): A         = visitBlock(node)
    def visitCommentBlock(node: CommentBlock): A   = visitBlock(node)
    def visitPassBlock(node: PassBlock): A         = visitBlock(node)
...
object AstVisitor:
    ...
    def visit[A](node: AstNode, visitor: AstVisitor[A]): A = node match
        ...
        case n: LiteralBlock    => visitor.visitLiteralBlock(n)
        case n: SidebarBlock    => visitor.visitSidebarBlock(n)
        case n: ExampleBlock    => visitor.visitExampleBlock(n)
        case n: QuoteBlock      => visitor.visitQuoteBlock(n)
        case n: OpenBlock       => visitor.visitOpenBlock(n)
        case n: CommentBlock    => visitor.visitCommentBlock(n)
        case n: PassBlock       => visitor.visitPassBlock(n)
    ...
    def children(node: AstNode): List[AstNode] = node match
        ...
        case _: LiteralBlock    => Nil // verbatim content, no child nodes
        case _: CommentBlock    => Nil // discarded content, no child nodes
        case _: PassBlock       => Nil // raw content, no child nodes
        case sb: SidebarBlock   => sb.blocks
        case eb: ExampleBlock   => eb.blocks
        case qb: QuoteBlock     => qb.blocks
        case ob: OpenBlock      => ob.blocks
